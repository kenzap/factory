import { __html, html, onClick, simulateClick } from '@kenzap/k-cloud';
...
export class ProductFiles {
    ...
    init() {
        ...
        document.querySelector("#cad_files").value = this.state.product.cad_files;
    ...
    listeneres() {
        ...
        self.state.cb = () => self.refreshList();
    ...
    refreshList() {
        ...
        if (!self.state.product.cad_files) self.state.product.cad_files = [];
        if (!self.state.product.cad_files.length) { document.querySelector('.cad_files_cont').innerHTML = '<div class="fst-italic>' + __html("No files uploaded yet.") + '</div>'; return; }

        document.querySelector('.cad_files_cont').innerHTML = self.state.product.cad_files.map(cad => {
            ...
            return `
                <div class="d-flex align-items-center flex-column border-1 bd-highlight border border-secondary border-dashed rounded p-2 mb-2 me-3">
                    <div class="d-block">
                        ...
                        <span class="remove po" title="${__html('Remove')}" data-id="${cad.id}" data-_id="${cad._id}">×</span>
                    </div>
                    <div class="d-block file-title text-truncate">${html(cad.name)}</div>
                </div>`;
        ...
        onClick(".cad_files_cont .remove", e => {
            ...
            self.state.product.cad_files = self.state.product.cad_files.filter(cad => cad.id != e.currentTarget.dataset.id);

            if (!self.state.filesToRemove) self.state.filesToRemove = [];

            self.state.filesToRemove.push(e.currentTarget.dataset._id);

            self.state.ProductSketch.sketchMode();
