import { __html, getStorage, onChange, onClick, spaceID } from '@kenzap/k-cloud';
import { Arc } from "../_/_2d_arc.js";
import { Polyline } from "../_/_2d_polyline.js";
import { degToRad, getProductId, log, onlyNumbers, unescape } from "../_/_helpers.js";
import { SketchControls } from "../_/_sketch/_controls.js";
import { hasRenderFiles } from "../_/_sketch/_helpers.js";
import { renderPreview } from "../_/_sketch/_rendering.js";
import { SketchUpload } from "../_/_sketch/_upload.js";
import { Upload } from "../_/_upload.js";
...
export class ProductSketch {
    ...
    init() {
        ...
        // sketch controls
        this.state.SketchUpload = new SketchUpload(this.state);

        this.state.SketchControls = new SketchControls(this.state);
                        ...
                        <div class="p-img-cont sketch-cont float-start" data-mode="preview" style="max-width:100%;">
                            <div data-index="sketch${i}" class="pe-0 border border-secondary-" style="position: relative;">
                                ...
                                <sketch-render class="d-none">
                                    ...
                                    <input class="camera-view form-text text-end d-none-" value="${this.state.sketch.camera.p1 + ',' + this.state.sketch.camera.p2 + ',' + this.state.sketch.camera.p3}">
    ...
    listeners() {
        ...
        // update camera view
        onChange('.camera-view', e => { renderPreview(self.state, e, "cam"); });
        ...
        d.querySelector("#svg").addEventListener('mousedown', function (e) {
            ...
            self.state.sketch.mousedown = true;
            ...
            if (self.mode == 'editing' && self.type == "polyline") { self.polyline = new Polyline({ points: [{ x: e.offsetX, y: e.offsetY }], mode: 'editing', id: self.state.sketch.last.id, rect: self.state.sketch.rect, annotation: document.querySelector('#svg g[data-id="' + self.state.sketch.last.id + '"]') ? document.querySelector('#svg g[data-id="' + self.state.sketch.last.id + '"]').dataset.annotation : 'line' }); self.type = "polyline"; }

            if (self.mode == 'editing' && self.type == "arc") { self.arc = new Arc({ params: { x: e.offsetX, y: e.offsetY, radius: 25, startAngle: 60, endAngle: 170 }, mode: 'editing', id: self.state.sketch.last.id, rect: self.state.sketch.rect, annotation: document.querySelector('#svg g[data-id="' + self.state.sketch.last.id + '"]') ? document.querySelector('#svg g[data-id="' + self.state.sketch.last.id + '"]').dataset.annotation : 'line' }); self.type = "arc"; }
        });
        ...
        d.querySelector("#svg").addEventListener('mousemove', function (e) {
            ...
            if (self.arc && self.mode == 'editing' && self.type == "arc") { self.arc = new Arc({ params: { x: e.offsetX, y: e.offsetY, radius: 25, startAngle: 60, endAngle: 170 }, mode: 'editing', id: self.state.sketch.last.id, rect: self.state.sketch.rect }); self.type = "arc"; }
        ...
        d.querySelector("#svg").addEventListener('mouseup', function (e) {
            ...
            self.state.sketch.mousedown = false;
            ...
            if ((self.polyline || self.arc || self.state.sketch.arrow) && (self.mode == 'annotate' || self.mode == 'editing')) {
                self.loadFields();
            }

            self.polyline = null;
            self.arc = null;
            // self.state.sketch.arrow = null;
            ...
            if (document.querySelector('#svg rect')) for (let el of document.querySelectorAll('#svg rect')) {
                ...
                el.onmouseover = (e) => {
                    ...
                    if (self.state.sketch.mousedown) return;

                    self.state.sketch.rect = e.currentTarget.dataset.rect;
                    self.state.sketch.last.id = e.currentTarget.dataset.id.replace('start', '').replace('end', '');
            ...
            if (document.querySelector('#svg polyline')) for (let el of document.querySelectorAll('#svg polyline')) {
                ...
                el.onmouseover = (e) => {
                    ...
                    self.state.sketch.last.id = e.currentTarget.dataset.id;
                };
            }
        ...
        document.querySelector('#svg').onmouseover = (e) => {
            ...
            self.state.sketch.hover = true;
        };

        document.querySelector('#svg').onmouseleave = (e) => {

            self.state.sketch.hover = false;
        };
        ...
        let keypress = (e) => {
            ...
            if (e.which != 8 || self.mode != 'editing' || !self.state.sketch.hover) return;

            // delete lines
            if (document.querySelector('#svg g[data-id="' + self.state.sketch.last.id + '"]')) document.querySelector('#svg g[data-id="' + self.state.sketch.last.id + '"]').remove();
            if (document.querySelector('.input-fields #field' + self.state.sketch.last.id)) document.querySelector('.input-fields #field' + self.state.sketch.last.id).remove();
            if (document.querySelector('.svg-input[data-id="' + self.state.sketch.last.id + '"]')) document.querySelector('.svg-input[data-id="' + self.state.sketch.last.id + '"]').remove();

            let id = self.state.sketch.last.id;

            this.state.product.input_fields = this.state.product.input_fields.filter(obj => obj.id != id);
    ...
    loadSketch() {
        ...
        let image_url = [getStorage() + '/S' + sid + '/sketch-' + id + '-1-500x500.webp?' + product.updated, getStorage() + '/S' + sid + '/sketch-' + id + '-1-500x500.jpeg?' + product.updated];
        ...
        if (hasRenderFiles(this.state)) image_url = ['https://render.factory.app.kenzap.cloud/' + id + '-polyester-2h3-1500.webp'];
        ...
        if (self.state.product.input_fields) {

            self.sketchMode('editing');

            self.state.product.input_fields.forEach(obj => {
    ...
    loadFields() {
        ...
        // console.log(self.state.product.input_fields);
        if (!self.state.product.input_fields) self.state.product.input_fields = [];
        ...
        if (document.querySelector('#svg g')) for (let el of document.querySelectorAll('#svg g')) {
            ...
            let obju = self.state.product.input_fields ? self.state.product.input_fields.filter(o => o.id == id)[0] : "";
            ...
            if (!obju.id) {
                ...
                self.state.product.input_fields.push(obj);

                // update params and points for previously created annotates
            } else {

                self.state.product.input_fields = self.state.product.input_fields.map(o => o.id == obj.id ? obj : o);
            }
        ...
        self.state.Upload = new Upload(self.state);
    ...
    saveState(e) {
        ...
        // clear empty fields
        self.state.product.input_fields = self.state.product.input_fields.filter(obj => obj.id);

        // update state, same values from html input fields to array
        self.state.product.input_fields.forEach(obj => { if (obj.id == e.currentTarget.dataset.id) { obj[e.currentTarget.dataset.key] = e.currentTarget.value; } });
    }
    ...
    sketchMode(mode) {
        ...
        if (!mode) mode = hasRenderFiles(this.state) ? 'annotate' : 'upload';
        ...
        if (mode == 'annotate') {
            ...
            this.state.SketchControls.init("");
        }
    ...
    getFields() {
        ...
        return this.state.product.input_fields;
    }
        ...
        return `
                <div id="field${obj.id}" class="d-flex flex-row bd-highlight mb-0" data-type="${obj.type}"  data-points="${obj.points}" data-params="${escape(JSON.stringify(obj.params ? obj.params : {}))}">
                    ...
                    ${obj.annotation && obj.annotation.includes('arrow-info') ? `
                        <div class="me-3">
                            <input type="text" class="form-control form-control-sm field-info input-field" placeholder="${__('Info note goes here')}" value="${obj.note ? obj.note : ""}" data-key="note" data-id="${obj.id}" style="width:398px;"></input>
                        ...
                        <div class="me-3 d-none">
                            ...
                            <p class="form-text">${__('attach file')}</p>
                        </div>
                        ...
                        <input type="hidden" class="form-control form-control-sm field-default input-field" placeholder="${__('0')}" value="${obj.default}" data-key="default" data-id="${obj.id}"></input>
                        <input type="hidden" class="form-control form-control-sm field-min input-field" placeholder="${__('0')}" value="${obj.min}" data-key="min" data-id="${obj.id}"></input>
                        <input type="hidden" class="form-control form-control-sm field-max input-field" placeholder="${__('1000')}" value="${obj.max}" data-key="max" data-id="${obj.id}"></input>

                        ` : `

                        <div class="me-3">
                            <input type="text" class="form-control form-control-sm field-default input-field" placeholder="${__('0')}" value="${obj.default}" data-key="default" data-id="${obj.id}"></input>
                            <p class="form-text">${__('default value')}</p>
                        </div>
                        <div class="me-3">
                            <input type="text" class="form-control form-control-sm field-min input-field" placeholder="${__('0')}" value="${obj.min}" data-key="min" data-id="${obj.id}"></input>
                            <p class="form-text">${__('min value')}</p>
                        </div>
                        <div class="me-3">
                            <input type="text" class="form-control form-control-sm field-max input-field" placeholder="${__('1000')}" value="${obj.max}" data-key="max" data-id="${obj.id}"></input>
    ...
    isStaticImage() {
        ...
        if (!hasRenderFiles(this.state) && document.querySelector(".images-sketch0").src.indexOf('placeholder') != -1) return false;
    }
}