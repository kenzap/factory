...
class TaskUpdatesSSEService {
    ...
    async connectWithPost(token) {
        ...
        try {
            ...
            const response = await fetch('/api/task-updates/connect', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                throw new Error('Failed to connect to task updates');
            }

            if (!response.body) {
                throw new Error('Task updates stream body is not available');
            }
    ...
    setupEventStream(reader, token) {
        ...
        const readStream = () => {
            reader.read().then(({ done, value }) => {
                ...
                console.error('Task SSE stream read error:', error);
                this.isConnected = false;
                this.reader = null;
                this.attemptReconnect(token);
            });
        ...
        try {
            ...
            this.listeners.forEach((callback) => {
                try {
                    ...
                    console.error('Error in task SSE listener:', error);
                }
            });
        } catch (error) {
...
export const taskUpdatesSSE = new TaskUpdatesSSEService();
