import { spaceID } from '@kenzap/k-cloud';
import * as THREE from 'three';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { MTLLoader } from 'three/addons/loaders/MTLLoader.js';
import { OBJLoader } from 'three/addons/loaders/OBJLoader.js';
import { getStorage } from "../_helpers.js";
...
export class ProductViewer {
    ...
    constructor(state) {
        ...
        // console.log('ProductViewer', this.state.sketch.offset);
    ...
    init() {
        ...
        let mtlFileExists = this.state.product.cad_files.find(file => file.name.endsWith('.mtl'));
        let objFileExists = this.state.product.cad_files.find(file => file.name.endsWith('.obj'));

        if (!mtlFileExists || !objFileExists) { self.state.viewerFirstLoad = true; return; }

        const mtlFile = getStorage() + '/S' + spaceID() + '/' + this.state.product.cad_files.find(file => file.name.endsWith('.mtl')).id + '.mtl';
        self.state.objFile = getStorage() + '/S' + spaceID() + '/' + this.state.product.cad_files.find(file => file.name.endsWith('.obj')).id + '.obj';
    ...
    loadObjFile() {
        ...
        self.state.objLoader.load(self.state.objFile, (object) => {
            ...
            // console.log('Object loaded', self.state.sketch.offset);

            // Center the object
            const box = new THREE.Box3().setFromObject(object);
            const center = box.getCenter(new THREE.Vector3());
            object.position.sub(center);
            if (self.state.sketch.offset) {
                object.position.x += (box.max.x - box.min.x) * self.state.sketch.offset.x;
                object.position.y += (box.max.y - box.min.y) * self.state.sketch.offset.y;
            }
            ...
            self.state.camera.fov = self.state.sketch.fov;

            // const minZ = box.min.z;
            // const cameraToFarEdge = minZ < 0 ? -minZ + self.state.sketch.position.z : self.state.sketch.position.z - minZ;
            ...
            if (self.state.viewerFirstLoad) {
                ...
                // console.log('Applying viewer parameters', self.state.sketch);
                self.state.camera.fov = self.state.sketch.fov;
                self.state.camera.position.set(self.state.sketch.position.x, self.state.sketch.position.y, self.state.sketch.position.z);
                self.state.camera.rotation.set(self.state.sketch.rotation.x, self.state.sketch.rotation.y, self.state.sketch.rotation.z);
                self.state.camera.updateProjectionMatrix();
                self.state.viewerFirstLoad = false;

                // If the position is not set, center the camera on the object. obj and mtl were just uploaded.
                if (self.state.sketch.position.x == 0 && self.state.sketch.position.y == 0 && self.state.sketch.position.z == 0) {
    ...
    refresh() {
        ...
        // console.log('refresh viewer', self.state.sketch.offset);
    ...
    view() {
        ...
        if (!this.state.product.cad_files.find(file => file.name.endsWith('.obj'))) return;
    ...
    captureParameters() {
        ...
        if (this.state.allowCameraUpdate) this.state.timeOut = setTimeout(() => {
            ...
            this.state.sketch.camera.p1 = parseFloat(this.state.camera.position.x);
            this.state.sketch.camera.p2 = parseFloat(this.state.camera.position.y);
            this.state.sketch.camera.p3 = parseFloat(this.state.camera.position.z);

            // if (!this.state.product.sketch.viewer) this.state.product.sketch.viewer = { position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 }, fov: 75 };

            this.state.sketch.position.x = parseFloat(this.state.camera.position.x);
            this.state.sketch.position.y = parseFloat(this.state.camera.position.y);
            this.state.sketch.position.z = parseFloat(this.state.camera.position.z);
            this.state.sketch.fov = parseFloat(this.state.camera.fov);
        ...
        // this.state.product.sketch.viewer = this.state.viewerParameters;
