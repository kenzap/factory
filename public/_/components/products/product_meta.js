...
import { __attr, __html, attr, languages, log, makeNumber, onChange, onClick } from "../../helpers/global.js";
import { ProductSlug } from "./modal_product_slug.js";
...
export class ProductMeta {
    ...
    constructor(product, locales, settings) {
        ...
        if (!this.product.locales) this.product.locales = {};

        this.locales.unshift({ locale: 'default', language: 'Default' });
    ...
    view() {
        ...
        document.querySelector('product-meta').innerHTML = `
            <h4 id="h-description" class="card-title mb-3 d-flex justify-content-between align-items-center">
                ${__html('Description')}
            ...
            if (!this.product.locales[locale.locale]) this.product.locales[locale.locale] = {};
            ...
            return `
                <div id="meta-locale-${locale.locale}" class="meta-locale ${id.length > 0 ? "d-none" : ""}">
                    <div class="mb-3 mww">
                        ...
                        <input type="text" class="form-control ${id.length == 0 ? "inp" : ""}" id="p-title${id}" placeholder="${__html('Metal flashing..')}" value="${this.product.locales[locale.locale]['title'] || ""}" maxlength="200">
                        <div class="d-flex justify-content-between form-text">
                            ...
                            <p class="mb-0 mt-0 fst-italic form-text text-decoration-underline po d-flex align-items-center edit-product-slug">
                                ...
                                <span class="product-slug-default">/${this.product.slugs.default ? this.product.slugs.default : __attr('metal-flashing-1')}/</span>
                            </p>
                        </div>
                    </div>
                    <div class="mb-3 mww">
            ...
            `<option value="${attr(group.id)}" ${this.product['group'] == group.id ? "selected='true'" : ""}">${__html(group.name)}</option>`
                    ...
                    <p class="form-text">${__html('Unite products by group.')}</p>
                </div>
            </div>
    ...
    bind() {
        ...
        d.querySelector("#p-sdesc").value = this.product.sdesc;
        d.querySelector("#p-ldesc").value = this.product.ldesc;
        d.querySelector("#priority").value = this.product.priority;
        d.querySelector("#linked_products").value = this.product.linked_products;
    ...
    editor() {
        ...
        [...document.querySelectorAll(".richtext-input")].forEach((el) => {
            ...
            editor.container.firstChild.innerHTML = this.product.locales[el.dataset.locale]["ldesc"] || "";
        });
    }
}