...
class MetalLog {
    ...
    renderRecords() {
        ...
        // Group entries by coating
        const entriesByCoating = entriesToShow.reduce((groups, entry) => {
            const coating = entry.coating || 'No Coating';
            if (!groups[coating]) {
                groups[coating] = [];
            }
            groups[coating].push(entry);
            return groups;
        }, {});
    ...
    renderProductName(entry) {
        ...
        if (entry.type === 'metal') {
            return `<span class="product-name">${entry.width} x ${Number(entry.length).toLocaleString()} ${getDimUnit(this.settings)}</span>`;
        }
