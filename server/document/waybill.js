...
async function viewWaybill(_id, user, lang) {
    ...
    try {
        ...
        let data = await getDocumentData(db, _id, user, locale);

        data.lang = lang;

        let waybill = data.settings?.waybill_document_template || "";
        ...
        data.waybill_items_table = getWaybillItemsTable(data.settings, data.order);
    ...
    app.get('/document/waybill/', async (req, res) => {
        try {
            ...
            if (req.query.email) {
                ...
                req.query.email = "pavel@kenzap.com";
